...
import { Spaceship, Direction, Movement } from '../objects/spaceship';
...
class ShooterScene extends Scene {
  ...
  preload(): void {
    ...
    this.load.image('ufo', 'assets/img/ufo.png');
  }
  ...
  create(): void {
    ...
    this.add
      ...
      .setOrigin(0, 0)
      .setScale(this.game.canvas.height / this.textures.get('space').getSourceImage().height);
    ...
    this.physics.add.collider(this.spaceShip, this.meteors, this.gameOver, undefined, this);
  }

  incrementBullets(): void {
    if (this.bulletState < this.BULLET_MAX) this.bulletState += 1;
  }
  ...
  handleMeteors(): void {
    if (this.time.now > this.meteorTime) {
      ...
      if (meteor) {
        ...
        this.meteorTime = this.time.now + 500 + (250 / this.speedMultiplier) * Math.random();
      }
    }
  }
